...
<h1>Hello, user {storephone}</h1>
  ...
  <div>
    <button class="enter-button" type="button" on:click={infohandleSubmit} disabled={!storephone}>Get info user</button>
...
<button class="enter-button" type="button" on:click={() => goto('/')} on:click={Exit}>Exit from account</button>
</div>
...
.button-container {
  ...
  .container {
    ...
    width: 80%;            /* Adjust as needed, responsive layout */
